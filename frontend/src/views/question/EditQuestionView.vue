<template>
  <div class="bg-base-300 rounded-2xl lg:w-3/5 m-auto mt-10">
    <div class="text-center font-bold text-6xl pt-10">Chỉnh câu hỏi</div>
    <vAddQuestionVue :data="data" />

    <!-- button -->
    <div class="space-x-5 mx-5 pb-5 text-center">
      <button v-if="!loading" @click="updateQ()" class="w-32 btn btn-outline btn-info">
        Cập nhật
      </button>
...
async function updateQ() {
  ...
  try {
    const id = await useQuestion.updateQ(data.value);
    if (id) {
