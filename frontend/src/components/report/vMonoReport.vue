  ...
  <div v-if="data.blog.length">
    ...
    <div class="w-full border-2 rounded-2xl bg-base-200 m-5">
      <div class="flex m-5 space-x-5 overflow-clip">
        <router-link :to="`/readblog/${data._id}`" class="btn btn-ghost text-2xl">{{
          data.blog[0].title || "RFWHFGO"
        }}</router-link>
      </div>
      ...
      <transition name="bounce">
        <div
          v-if="
            useReport.ListReport.length && useReport.ListReport[0].id_blog == data._id
          "
        >
          <div v-for="report in useReport.ListReport" :key="report.id">
            <vMonoContentReportVue :data="report" />
          </div>
        </div>
      </transition>
    </div>
...
<script setup>
import vMonoContentReportVue from "./vMonoContentReport.vue";
import { ref } from "vue";
...
async function readReport(id) {
  try {
    loading.value = true;
    await useReport.getListReportById("blog", id);
  } catch (error) {
    console.log(error);
